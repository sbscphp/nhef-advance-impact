...
class PledgeService
{
    ...
    public function createPledge(User $user, array $validated, Request $request): array
    {
        ...
        return DB::transaction(function () use ($user, $campaign, $validated, $frequency, $installmentCount, $schedule, $request): array {
            ...
            $initialization = $this->initializePaymentFor($user, $pledge, $firstInstallment, $validated['payment_method'] ?? null);
            ...
            GeneralHelper::storeAuditLog(
                ...
                $request,
                $user->uuid,
                ['campaign_uuid' => $campaign->uuid, 'frequency' => $frequency->value, 'total_amount' => $validated['total_amount']],
                $user->displayName().' created a pledge for '.$campaign->title.'.',
                Pledge::class,
                $pledge->uuid,
            ...
            return [
                'pledge' => $this->pledgeRepository->loadFresh($pledge, ['campaign', 'installments', 'payments']),
    ...
    public function verifyPayment(string $reference, Request $request): array
    {
        ...
        if ($result['status'] !== 'successful') {
            ...
            GeneralHelper::storeAuditLog(
                ...
                $request,
                $payment->user->uuid,
                ['reference' => $reference, 'gateway_status' => $result['status']],
        ...
        return DB::transaction(function () use ($payment, $result, $reference, $request): array {
            ...
            GeneralHelper::storeAuditLog(
                ...
                $request,
                $payment->user->uuid,
                ['reference' => $reference, 'amount' => (string) $payment->amount],
            ...
            $this->notifyPaymentSucceeded($payment->user, $pledge, $campaign);

            return [
                'payment' => $payment,
                'pledge' => $this->pledgeRepository->loadFresh($pledge, ['campaign', 'installments']),
            ];
        });
    }
     ...
     */
    private function initializePaymentFor(User $user, Pledge $pledge, PledgeInstallment $installment, ?string $paymentMethod): array
    {
    ...
    private function notifyPaymentSucceeded(User $user, Pledge $pledge, Campaign $campaign): void
    {
